...
// output the sum of the trailheads' (the 0s) RATING (how many distinct paths can be found to reach a 9)
...
int findTrails(int map[MAP_HEIGHT][MAP_WIDTH], int y, int x) {
    ...
    // PART 2: we're counting all distinct paths, so we don't need peaks found
    return step(map, y, x);
}
 ...
 * return : the number of trails possible from this trailhead
 */
int step(int map[MAP_HEIGHT][MAP_WIDTH], int y, int x) {
