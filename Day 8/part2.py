...
# PART 2: antinodes can now also spawn in line with any two antennae in a line
...
def determine_antinodes(map, anti_map, freq):
    ...
    # PART 2: this step needs to continue until the end of the grid. also needs to include the antennae themselves.
    for i in range(len(tenna)):
        for j in range(i+1, len(tenna)):
            # if tenna 1 is farther left than 2
            if tenna[i][0] < tenna[j][0]:
                x_step = tenna[j][0] - tenna[i][0]
            else:
                x_step = tenna[i][0] - tenna[j][0]

            # if tenna 1 is higher up than 2
            if tenna[i][1] < tenna[j][1]:
                y_step = tenna[j][1] - tenna[i][1]
            else:
                y_step = tenna[i][1] - tenna[j][1]

            x1 = tenna[i][0]
            x2 = tenna[j][0]
            y1 = tenna[i][1]
            y2 = tenna[j][1]
            while (x1 >= 0 and x1 < len(map) and y1 >= 0 and y1 < len(map)) or (x2 >= 0 and x2 < len(map) and y2 >= 0 and y2 < len(map)):
                # mark antinodes
